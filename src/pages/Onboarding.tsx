...
export function Onboarding({ onFinish, onDemo, data, actions, syncStatus }: Props) {
  ...
  const isSignedIn = syncStatus.authenticated;
  ...
  async function handleAuth() {
    ...
    try {
      if (authMode === "login") {
        await actions.signIn(authEmail, authPassword);
      } else {
        await actions.signUp(authEmail, authPassword);
      }
    } catch (err: unknown) {
      setAuthError(err instanceof Error ? err.message : "Erro ao autenticar. Tente novamente.");
    } finally {
      setBusy(false);
    }
  }
      ...
      <div className="mx-auto max-w-md px-5 pt-[max(28px,env(safe-area-inset-top))]">
        ...
        {step === 0 && (
          <div className="space-y-4">
            {isSignedIn ? (
              <div className="flex items-center gap-2 rounded-2xl bg-green-50 px-4 py-3 text-sm font-semibold text-success">
                <CheckCircle2 size={16} /> Conta conectada — dados a sincronizar
              </div>
            ) : (
              <>
                {/* Mode tabs */}
                <div className="flex rounded-2xl border border-outline bg-slate-50 p-1">
                  <button
                    type="button"
                    className={`flex-1 rounded-xl py-2.5 text-sm font-semibold transition-all ${authMode === "login" ? "bg-white text-ink shadow-sm" : "text-muted"}`}
                  ...
                  <button
                    type="button"
                    className={`flex-1 rounded-xl py-2.5 text-sm font-semibold transition-all ${authMode === "register" ? "bg-white text-ink shadow-sm" : "text-muted"}`}
                ...
                <div className="form-card space-y-3">
                  ...
                  {authError && (
                    <p className="rounded-xl bg-red-50 px-3 py-2 text-sm text-danger">{authError}</p>
                  )}
                  <button
                    type="button"
                ...
                <button
                  type="button"
                  disabled={busy}
                  className="btn-secondary w-full justify-center gap-2 py-3 disabled:opacity-50"
            ...
            <div className="space-y-2 pt-1">
              <button
                ...
                onClick={() => setStep(1)}
              >
                Continuar sem conta <ArrowRight size={19} />
              </button>
              <button
                type="button"
                onClick={onDemo}
